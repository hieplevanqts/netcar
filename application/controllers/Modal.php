...
// quên mật khâu
 ...
 // chon mau sac trong chi tiet sp
    public function get_option_color(){
        $data = array();
        $this->load->model('f_productmodel');
        $data['id'] = $id = $_POST['id'];
        $data['qty'] = $_POST['qty'];
        $data['p_color'] = $this->system_model->get_data('p_color',array(
            'pro_id' => $id
        ));
         $data['size'] = $this->system_model->get_data('product_to_size',array(
            'id_product' => $id
        ));
         $data['size'] = $this->f_productmodel->getProductSize($id);
        // var_dump($data['size']);die;

        $this->load->view('modal/view_chonmau',$data);
    }
