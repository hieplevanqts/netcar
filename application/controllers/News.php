...
class News extends MY_Controller
{
    ...
    public function new_bycategory($alias){
        ...
        $data['list1'] = $this->f_newsmodel->getNewsByCategory($current->id,null, null);

        $config['base_url'] = base_url('danh-muc-tin/'.$alias.'.html');
        $config['page_query_string'] = TRUE;
        $config['enable_query_string'] = TRUE;
        $config['total_rows'] = count($data['list1']);
        ...
        $data['lists2'] = $this->system_model->get_data('news',array(
            'lang' => $this->language,
            'focus'=>1,
        ),array(),false,2,0);

        $data['lists3'] = $this->system_model->get_data('news',array(
            'lang' => $this->language,
            'focus'=>1,            
        ),array(),false,3,2);

        $data['lists4'] = $this->system_model->get_data('news',array(
            'lang' => $this->language,
            'focus'=>1,
        ),array(),false,10,5);
        ...
        $data['news_left'] = $this->load->widget('news_left');
    ...
    public function detail($alias){
        // $this->Check_alias($alias);
        $data['news'] = $new = $this->f_newsmodel->getFirstRowWhere('news',array(
            'alias'=>$alias,
        ));
        ...
        $data['cate_current'] = $current = $this->f_newsmodel->get_data('news_category',array(
            ...
            array(),true);
        $data['news_tuyendung'] = $current = $this->f_newsmodel->get_data('news',array(
            'category_id'=>4),
        array(),true);
    ...
    public function new_all(){
        ...
        $data = array();
        //if(empty($data['cate_current'])){ redirect(base_url('404_override')); }
        $data['cate'] = $this->system_model->get_data('news',array(
            'lang' => $this->language
        ),array());
        ...
        $config['page_query_string'] = TRUE;
        $config['enable_query_string'] = TRUE;
        $config['total_rows'] = count($data['cate']);
        $config['per_page'] = 4; // xác định số record ở mỗi trang
        $config['uri_segment'] = 4; // xác định segment chứa page number
        $config=array_merge($config,$this->pagination_config);
        $this->pagination->initialize($config);
        ...
        $seo=array();
        $this->LoadHeader2(null,$seo,true);
        $this->load->view('news/news_all',@$data);
        $this->LoadFooter();
    }
}