...
public function count_ProbyCate($id)
    ...
    public function getProbyCate2($id, $order, $limit, $offset)
    {
      $query = $this->db->select('product.*,product_to_category.id_category,product_category.parent_id')
            ->from('product')
            ->join('product_category', 'product.category_id=product_category.parent_id','left')
            ->join('product_to_category', 'product.id=product_to_category.id_product','left')
            ->where('product_to_category.id_category', @$id)
    ...
    // 
    public function getProbyCate3($id, $order, $limit, $offset)
    {
      $query = $this->db->select('product_phutung.*,product_to_phutung.id_dm_phutung,product_danhmuc_phutung.parent_id')
            ->from('product_phutung')
            ->join('product_danhmuc_phutung', 'product_phutung.id_dm_phutung=product_danhmuc_phutung.parent_id','left')
            ->join('product_to_phutung', 'product_phutung.id=product_to_phutung.id_phutung','left')
            ->where('product_to_phutung.id_dm_phutung', @$id)
            ->order_by('product_phutung.id','desc')
            ->limit($limit, $offset)
            ->get();
        return $query->result();
    }
